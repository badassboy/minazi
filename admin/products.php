		...
		<div class="main">
			...
			<main class="content">
				...
				<?php endif ?>
					...
					<button class="btn btn-sm btn-primary float-end" onclick="window.location.href='products-add.php'">Add Product</button>
							...
							<table  id="example" class="table table-hover table-striped my-0" id="example" style="width:100%">
									...
									<tbody>
										...
										<tr>
											...
											<td class="d-none d-xl-table-cell"><img src="../en/<?=$prod['p_ft_photo']?>" alt="<?=$prod['p_name']?>" width="50px"></td>
											<td class="d-none d-xl-table-cell text-wrap"><?=$prod['p_name']?></td>
											<td class="text-center"><?=$prod['p_old_price']?></td>
											<td class="text-center"><?=$prod['p_current_price']?></td>
											<td class="text-center"><?=$prod['p_qty']?></td>
											<td class="text-center text-danger"><?=$prod['p_remaining']?></td>
											<td class="text-center"><?=blStatus($prod['p_is_featured'])?></td>
											<td class="text-center"><?=ckStatus($prod['p_is_active'])?></td>
											<td><?=catGroup($prod['cat_id'])?></td>
											<td>
											<div class="dropdown">
